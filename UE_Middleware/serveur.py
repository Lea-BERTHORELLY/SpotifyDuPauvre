...
class PlayerI(MusicPlayer.Player):
    ...
    def SearchArtist(self, s, current=None):
        Artistes=[]
        cursorDatabase.execute("SELECT Titre, Artistes, Album FROM Musiques WHERE Artistes LIKE '%"+s+"%' ORDER BY Artistes")
        for row in cursorDatabase:
            print(row[0])
    ...
    def SearchTitle(self, s ,current=None):
        Titres=[]
        cursorDatabase.execute("SELECT Titre, Artistes, Album FROM Musiques WHERE Titre LIKE '%"+s+"%' ORDER BY Titre")
        for row in cursorDatabase:
            print(row[0])
